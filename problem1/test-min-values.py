...
from pyMetaheuristic.test_function import easom, ackley, axis_parallel_hyper_ellipsoid, beale, bohachevsky_1, bohachevsky_2, bohachevsky_3, booth, branin_rcos, bukin_6, cross_in_tray, de_jong_1, drop_wave, eggholder, goldstein_price, griewangk_8, himmelblau, holder_table, matyas, mccormick, levi_13, rastrigin, rosenbrocks_valley, schaffer_2, schaffer_4, schaffer_6, schwefel, six_hump_camel_back, styblinski_tang, three_hump_camel_back, zakharov
...
# Tests the algorithm with different target functions
...
def improved_grey_wolf_optimizer(initialize_random=False, pack_size = 25, min_values = [-100,-100], max_values = [100,100], iterations = 500, target_function = target_function, verbose = True, start_init = None, target_value = None):   
    ...
    # print("\n =================================================")
    # print(f"Initial position [{target_function.__name__}] :\n")
    # print(f"Alpha Position: {alpha}")
    # print(f"Beta Position:  {beta}")
...
def main():
	...
	parameters = {
		...
		'max_values': (10, 10),
		'iterations': 1000,
		'verbose': False, # change this to True to see the iteration number and the alpha position
		'start_init': None,
		'target_value': None,
		'initialize_random': False # change this to simulate random initialization
	}
 
	# Test the algorithm with different target functions from target_functions dictionary
	for target_function_name, target_function in target_functions.items():
		gwo = improved_grey_wolf_optimizer(target_function = target_function, **parameters) 
  
		# display the target function name and the minimum value found and its alpha position
		# print(f"\nTarget Function: {target_function_name}")
		# ? goldstein_price, schwefel, and styblinski_tang has the largest minimum value
		print(f"{target_function_name}: {gwo[:-1]} = {gwo[-1]}")
